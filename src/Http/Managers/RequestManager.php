...
namespace UdaraJay\Dunbar\Http\Managers;

use Illuminate\Support\Arr;
use GuzzleHttp\Client;
use GuzzleHttp\Exception\ClientException;
use UdaraJay\Dunbar\Http\ProxyAux;
use UdaraJay\Dunbar\Models\ProxyResponse;
use UdaraJay\Dunbar\Http\Exceptions\MissingClientSecretException;
...
class RequestManager {
    ...
    private function sendGuzzleRequest($method, $uriVal, $inputs) {
        ...
        $client = new Client([ 'verify' => $verifySSL]);
        ...
        if ($this->callMode === ProxyAux::MODE_TOKEN && $this->useHeader === true) {
            ...
            $options = Arr::add($options, 'headers', [ProxyAux::HEADER_AUTH => 'Bearer ' . $accessToken]);
        }

        if ($method === 'GET') {
            $options = Arr::add($options, 'query', $inputs);
        } else {
            $options = Arr::add($options, 'form_params', $inputs);
        }

        $request = $client->request($method, $uriVal, $options);
